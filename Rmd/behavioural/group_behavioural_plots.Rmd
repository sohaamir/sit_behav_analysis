...
Plot scatter plots against believability rating for real players
```{r, fig.width=12, fig.height=6}
# Create scatter plots showing individual data against player.real_players rating
# For Choice 1/2 Accuracy and Bet 1/2 Magnitude separately

# Calculate participant-level metrics
participant_data <- data %>%
  group_by(participant.label) %>%  # Using participant.label as requested
  summarize(
    # Choice accuracy values
    choice1_accuracy = mean(player.choice1_accuracy, na.rm = TRUE) * 100,
    choice2_accuracy = mean(player.choice2_accuracy, na.rm = TRUE) * 100,
    
    # Bet magnitude values
    bet1_magnitude = mean(player.bet1, na.rm = TRUE),
    bet2_magnitude = mean(player.bet2, na.rm = TRUE),
    
    # The predictor variable
    real_players_rating = mean(player.real_players, na.rm = TRUE)
  )

# Convert to long format for faceting
choice_data_long <- participant_data %>%
  select(participant.label, real_players_rating, choice1_accuracy, choice2_accuracy) %>%
  pivot_longer(
  ...
  mutate(condition = factor(condition, 
                           levels = c("choice1_accuracy", "choice2_accuracy"),
                           labels = c("Choice 1", "Choice 2")))

bet_data_long <- participant_data %>%
  select(participant.label, real_players_rating, bet1_magnitude, bet2_magnitude) %>%
  pivot_longer(
    cols = c(bet1_magnitude, bet2_magnitude),
    names_to = "condition",
    values_to = "magnitude"
  ) %>%
  mutate(condition = factor(condition, 
                           levels = c("bet1_magnitude", "bet2_magnitude"),
                           labels = c("Bet 1", "Bet 2")))

# Function to calculate statistics for annotations
get_stats_annotation <- function(data, x_var, y_var) {
  model <- lm(paste(y_var, "~", x_var), data = data)
  stats <- summary(model)
  ...
  return(sprintf(
    ...
    cor_test$conf.int[1],
    cor_test$conf.int[2],
    stats$coefficients[2, 1],
    stats$coefficients[2, 4],
    stats$r.squared,
    stats$fstatistic[2],
    stats$fstatistic[3],
    stats$fstatistic[1]
...
# Create choice accuracy faceted plot
choice_models <- choice_data_long %>%
  group_by(condition) %>%
  summarize(
    stats = list(get_stats_annotation(cur_data(), "real_players_rating", "accuracy")),
    .groups = "drop"
  )

choice_plot <- ggplot(choice_data_long, aes(x = real_players_rating, y = accuracy)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", color = "#1f77b4", fill = "lightgray") +
  facet_wrap(~ condition) +
  labs(
    title = "Effect of real_players_rating on choice accuracy",
    x = "real_players_rating",
    y = "Choice accuracy (%)"
  ) +
  theme_classic() +
  theme(
...
# Add statistics to each facet
choice_plot <- choice_plot +
  geom_text(
    data = choice_models,
    aes(label = stats),
    x = 20,  # Approximate middle of x-axis
    y = min(choice_data_long$accuracy) + 0 * (max(choice_data_long$accuracy) - min(choice_data_long$accuracy)),
    size = 2.5,
    hjust = 0.5
  )

# Create bet magnitude faceted plot
bet_models <- bet_data_long %>%
  group_by(condition) %>%
  summarize(
    stats = list(get_stats_annotation(cur_data(), "real_players_rating", "magnitude")),
    .groups = "drop"
  )

bet_plot <- ggplot(bet_data_long, aes(x = real_players_rating, y = magnitude)) +
  geom_point(color = "#2ca02c", size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", color = "#2ca02c", fill = "lightgray") +
  facet_wrap(~ condition) +
  labs(
    title = "Effect of real_players_rating on bet magnitude",
    x = "real_players_rating",
    y = "Bet magnitude"
  ) +
  theme_classic() +
...
bet_plot <- bet_plot +
  geom_text(
    ...
    aes(label = stats),
    x = 20, 
    y = min(bet_data_long$magnitude) + 0 * (max(bet_data_long$magnitude) - min(bet_data_long$magnitude)),
    size = 2.5,
    hjust = 0.5
  )

# Print the faceted plots
print(choice_plot)
print(bet_plot)

# Optional: save the plots
#ggsave("choice_real_players_faceted.png", choice_plot, width = 10, height = 6)
#ggsave("bet_real_players_faceted.png", bet_plot, width = 10, height = 6)
```
...
process_consensus_data <- function(data, outcome_type) {
  ...
  # Keep trial-level data instead of summarizing
  processed <- if(outcome_type == "switch") {
    filtered_data %>%
      mutate(
        outcome_value = player.switch_vs_stay * 100  # Keep as trial-level variable
      )
  } else {
    filtered_data %>%
      mutate(
        bet_difference = player.bet2 - player.bet1,
        outcome_value = bet_difference  # Keep as trial-level variable
      )
  }
...
consensus_model_formulas <- list(
  m1 = outcome_value ~ direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session),
  
  m2 = outcome_value ~ direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender),
  
  m3 = outcome_value ~ direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session),
  
  m4 = outcome_value ~ direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|consensus_numeric:participant.id_in_session),
  
  m5 = outcome_value ~ direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session) + 
         (1|consensus_numeric:participant.id_in_session)
)
...
process_switch_accuracy_data <- function(data, outcome_type) {
  ...
  # Create derived columns at trial level (no grouping)
...
get_switch_accuracy_formulas <- function(outcome_var) {
  formula_base <- as.formula(paste(
    outcome_var,
    "~ direction * consensus_numeric * choice_switch_across_trials + age_scaled"
  ))
...
bet_difference_formulas <- list(
  m1 = bet_difference ~ direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session),
  
  m2 = bet_difference ~ direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender),
  
  m3 = bet_difference ~ direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session),
  
  m4 = bet_difference ~ direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|consensus_numeric:participant.id_in_session),
  
  m5 = bet_difference ~ direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session) + 
         (1|consensus_numeric:participant.id_in_session)
)
...
get_reversal_formulas <- function(outcome_type) {
  ...
  list(
    m1 = as.formula(paste(dv, "~", iv, "*trial_to_reversal + age_scaled + 
                         (1|participant.id_in_session)")),
    
    m2 = as.formula(paste(dv, "~", iv, "*trial_to_reversal + age_scaled + 
                         (1|participant.id_in_session) + (1|gender)")),
    
    m3 = as.formula(paste(dv, "~", iv, "*trial_to_reversal + age_scaled + 
