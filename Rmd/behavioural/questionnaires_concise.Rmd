...
required_packages <- c("ggplot2", "tidyverse", "ggpubr", "rstatix", "ez", "lme4", "lmerTest", "car", "emmeans", "MuMIn", "psych", "interactions", "effects", "here", "lm.beta", "effectsize", "parallel", "gridExtra", "grid")
...
subscale_results <- map_df(questionnaires, function(quest) {
  if(!is.null(SUBSCALE_MAPPING[[quest]])) {
    map_df(SUBSCALE_MAPPING[[quest]], function(subscale) {
      result <- analyze_questionnaire_effects(subscale, participant_data)
      result$parent_scale <- quest
      return(result)
    })
  }
}) %>%
...
cat("Number of participants:", n_distinct(reversal_data$participant.id_in_session), "\n")
cat("Number of complete reversal windows:", n_distinct(reversal_data$group.trial_number), "\n")
...
for(var in questionnaire_vars) {
  # Create plots and get test results
  results <- plot_median_split_reversal(reversal_data, var, NULL, NULL, var)
  
  # Save plots in appropriate directories
  ggsave(
    filename = file.path(plot_choice_dir, paste0(var, "_choice_reversal.png")),
    plot = results$choice_plot,
    width = 5,
    height = 7
  )
  ...
  ggsave(
    ...
    plot = results$bet_plot,
    width = 5,
    height = 7
  )
  
  # Format and append results
  formatted_results <- format_results_reversal(results$test_results, 
                                             results$summary_data, 
                                             var)
  
  all_choice_results <- paste0(all_choice_results, formatted_results$choice_text, "\n\n")
  all_bet_results <- paste0(all_bet_results, formatted_results$bet_text, "\n\n")
  
  # Process subscales if they exist
  if(!is.null(SUBSCALE_MAPPING[[var]])) {
    for(subscale in SUBSCALE_MAPPING[[var]]) {
      subscale_results <- plot_median_split_reversal(reversal_data, 
                                                   subscale, 
                                                   NULL, 
                                                   NULL, 
                                                   subscale)
      ...
      ggsave(
        ...
        plot = subscale_results$choice_plot,
        width = 5,
        height = 7
      )
      ...
      ggsave(
        ...
        plot = subscale_results$bet_plot,
        width = 5,
        height = 7
      )
      
      # Format and append subscale results
      formatted_results <- format_results_reversal(subscale_results$test_results,
                                                 subscale_results$summary_data,
                                                 subscale)
