...
get_stars <- function(p) {
  if(is.na(p)) return(paste0("p = ", round(p, 2)))
  if(p < 0.001) return("***")
...
get_stars_with_d <- function(p, d) {
  stars <- if(is.na(p)) paste0("p = ", round(p, 2))
           else if(p < 0.001) "***"
           else if(p < 0.01) "**"
           else if(p < 0.05) "*"
...
process_consensus_data_groups <- function(data, outcome_type) {
  ...
  # Keep trial-level data instead of summarizing by participant
  processed <- if(outcome_type == "switch") {
    filtered_data %>%
      mutate(
        outcome_value = player.switch_vs_stay * 100  # Keep at trial level
      )
  } else {
    filtered_data %>%
      mutate(
        bet_difference = player.bet2 - player.bet1,
        outcome_value = bet_difference  # Keep at trial level
      )
  }
  
  # Check if age exists in the dataset and handle appropriately
  has_age <- "age" %in% colnames(filtered_data)
  ...
  processed_data <- processed %>%
    mutate(
      ...
      belief_group = factor(belief_group),  # Keep belief group
      participant.id_in_session = factor(participant.id_in_session)
    )
  
  # Only add scaled age if it exists
  if(has_age) {
    processed_data <- processed_data %>%
      mutate(age_scaled = scale(age)[,1])
  } else {
    # Create dummy age variable (all 0)
    processed_data <- processed_data %>%
      mutate(age_scaled = 0)
  }
...
consensus_model_formulas_groups <- list(
  m1 = outcome_value ~ belief_group * direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session),
  
  m2 = outcome_value ~ belief_group * direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender),
  
  m3 = outcome_value ~ belief_group * direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session),
  
  m4 = outcome_value ~ belief_group * direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|consensus_numeric:participant.id_in_session),
  
  m5 = outcome_value ~ belief_group * direction * consensus_numeric + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session) + 
         (1|consensus_numeric:participant.id_in_session)
)
...
process_switch_accuracy_data_groups <- function(data, outcome_type) {
  ...
  # Create derived columns at trial level
  filtered_data <- filtered_data %>%
    mutate(
      ...
      consensus_level = factor(consensus_level, levels = c("2:2", "3:1", "4:0")),
      direction = factor(direction, levels = c("Against group", "With group")),
      belief_group = factor(belief_group),  # Preserve belief group
      participant.id_in_session = factor(participant.id_in_session)
    )
  
  # Check if age exists and handle appropriately
  has_age <- "age" %in% colnames(filtered_data)
  if(has_age) {
    filtered_data <- filtered_data %>%
      mutate(age_scaled = scale(age)[,1])
  ...
  # Combine and prepare final dataset at trial level
...
get_switch_accuracy_formulas_groups <- function(outcome_var) {
  formula_base <- as.formula(paste(
    outcome_var,
    "~ belief_group * direction * consensus_numeric * choice_switch_across_trials + age_scaled"
  ))
...
create_switch_accuracy_plot_groups <- function(data, model_results, anova_results, params) {
  ...
  title <- ggdraw() + 
    draw_label(
      "Four-way Interaction: Belief × Direction × Consensus × Switch/Stay",
      fontface = 'bold', 
      size = 14, 
      x = 0.5, 
      y = 0.8
  ...
  # Add key interaction with p-value and effect size
  title <- title + 
    draw_line(x = c(0.3, 0.7), y = c(0.65, 0.65), size = 0.75) +
    draw_text(
      sprintf("Four-way Interaction: p = %.3f, η²ₚ = %.3f %s", 
             four_way_p, 
             four_way_eta,
             ifelse(four_way_p < 0.05, "(significant)", "(not significant)")),
      x = 0.5, 
      y = 0.58, 
      size = 12
    )
  ...
  title <- title + 
    draw_text(
      "Note: All p-values adjusted using Holm-Bonferroni correction\nEffect sizes shown as partial eta-squared (η²ₚ)",
      x = 0.5,
      y = 0.3,
      size = 8,
      hjust = 0.5,
  ...
  plot_with_title <- plot_grid(
    ...
    combined_plot,
    ncol = 1,
    rel_heights = c(0.2, 1)  # Reduced title height since there's less content
  )
...
# Run choice accuracy analysis
accuracy_results <- run_mixed_model_pipeline(
  data = data,
  preprocessing_function = function(x) process_switch_accuracy_data_groups(x, "accuracy"),
  model_formulas = get_switch_accuracy_formulas_groups("player.choice1_accuracy"),
  plot_function = create_switch_accuracy_plot_groups,
  analysis_name = "Choice Accuracy by Switch, Consensus, and Belief",
  output_path = here("output", "behav", "group_comparison", 
                    "choice_accuracy_by_switch_consensus_belief.txt"),
  additional_params = list(
    outcome_var = "player.choice1_accuracy",
    y_label = "Choice 1 accuracy (%)",
    is_accuracy = TRUE,
    emmeans_specs = c("belief_group", "direction", "consensus_numeric", "choice_switch_across_trials")
  )
)
...
process_bet_difference_data_groups <- function(data) {
  ...
  # Keep trial-level data (no grouping/summarizing)
...
bet_difference_formulas_groups <- list(
  m1 = bet_difference ~ belief_group * direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session),
  
  m2 = bet_difference ~ belief_group * direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender),
  
  m3 = bet_difference ~ belief_group * direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session),
  
  m4 = bet_difference ~ belief_group * direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|consensus_numeric:participant.id_in_session),
  
  m5 = bet_difference ~ belief_group * direction * consensus_numeric * switch_vs_stay + age_scaled + 
         (1|participant.id_in_session) + (1|gender) + 
         (1|direction:participant.id_in_session) + 
         (1|consensus_numeric:participant.id_in_session)
)
...
create_bet_difference_plot_groups <- function(data, model_results, anova_results, params) {
  ...
  for (group in c("Low Belief", "High Belief")) {
    ...
    y_min <- floor(min(group_data$mean_diff - group_data$se, na.rm = TRUE) * 10) / 10 - 0.1
    y_max <- ceiling(max(group_data$mean_diff + group_data$se, na.rm = TRUE) * 10) / 10 + 0.1
  ...
  title <- ggdraw() + 
    draw_label(
      sprintf("Four-way interaction: Belief Group moderates the effect of Direction, Consensus and Choice Switch on Bet Difference\nF_%s = %.2f, %s, η²ₚ = %.3f", 
             four_way_df, four_way_f, p_formatted, four_way_eta),
      fontface = 'bold',
      size = 12,
      x = 0.5,
      y = 0.85  # Position higher in the title area
    )
  ...
  if (nrow(significant_three_way) > 0) {
    ...
    title <- title + 
      draw_text(
        three_way_text,
        x = 0.5,
        y = 0.5,  # Position in middle of title area
        size = 9,
        hjust = 0.5
      )
  }
  ...
  title <- title + 
    draw_text(
      ...
      x = 0.5,
      y = 0.2,  # Position near bottom of title area
      size = 8,
      hjust = 0.5,
      fontface = "italic"
    )
  
  # Increase the height of the title section relative to the plot
  plot_with_title <- plot_grid(
    title, combined_plot,
    ncol = 1,
    rel_heights = c(0.4, 1)  # Fixed height ratio, increased from previous values
  )
...
process_reversal_data_groups <- function(data, outcome_type) {
  ...
  # Process data with additional filtering - keep at trial level
  processed_data <- data %>%
    group_by(participant.id_in_session) %>%
    mutate(
    ...
    ungroup() %>%  # Ungroup after calculating trial_to_reversal
  ...
  # Check if age exists and handle appropriately
  has_age <- "age" %in% colnames(processed_data)
  
  # Add demographic variables and convert to factors
  processed_data <- processed_data %>%
    mutate(
...
get_reversal_formulas_groups <- function(outcome_type) {
  ...
  list(
    m1 = as.formula(paste(dv, "~ belief_group *", iv, "*trial_to_reversal + age_scaled + 
                         (1|participant.id_in_session)")),
    
    m2 = as.formula(paste(dv, "~ belief_group *", iv, "*trial_to_reversal + age_scaled + 
                         (1|participant.id_in_session) + (1|gender)")),
    
    m3 = as.formula(paste(dv, "~ belief_group *", iv, "*trial_to_reversal + age_scaled + 
...
create_reversal_plot_groups <- function(data, model_results, anova_results, params) {
  ...
  type_values <- unique(data[[params$type_col]])
  
  # Process data for each type instead of each belief group
  for (type_val in type_values) {
    type_data <- data %>% 
      filter(!!sym(params$type_col) == type_val) %>%
      group_by(trial_to_reversal, belief_group) %>%
      summarise(
    ...
    processed_data[[as.character(type_val)]] <- type_data
  }
  
  # Get shared y-axis limits across all datasets
  y_limits <- get_shared_y_limits(
    processed_data[[as.character(type_values[1])]], 
    processed_data[[as.character(type_values[2])]], 
    "value",
    buffer = 0.1
  )
  
  # Create plots for each choice/bet type instead of belief group
  for (type_val in type_values) {
    type_key <- as.character(type_val)
    
    # Create plot for this type
    p <- ggplot(processed_data[[type_key]], 
           aes(x = trial_to_reversal, 
               y = value, 
               color = belief_group,  # Now color by belief group 
               group = belief_group)) +  # Group by belief group
      ggtitle(paste0(params$legend_title, ": ", type_val)) +
      ...
      # Now use different color scheme for belief groups
      scale_color_manual(values = c("Low Belief" = "#1E88E5", "High Belief" = "#D81B60")) +
      scale_x_discrete(labels = c("-3", "-2", "-1", "0", "1", "2", "3")) +
      scale_y_continuous(limits = c(y_limits$min, y_limits$max)) +
      labs(
        x = "Trial (relative to reversal)",
        y = params$y_label,
        color = "Belief Group"  # Change legend title to reflect belief groups
      ) +
      theme_classic() +
      theme(
        axis.title = element_text(size = 12),
  ...
  # Create combined plot with plots arranged by choice/bet type
  combined_plot <- plot_grid(
    plots[[as.character(type_values[1])]], 
    plots[[as.character(type_values[2])]], 
    nrow = 2,
    align = "v"
  )
