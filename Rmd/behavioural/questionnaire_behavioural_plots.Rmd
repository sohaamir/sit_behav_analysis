...
required_packages <- c("ggplot2", "tidyverse", "ggpubr", "rstatix", "ez", "lme4", "lmerTest", "car", "emmeans", "MuMIn", "psych", "interactions", "effects", "here", "lm.beta", "effectsize", "parallel", "gridExtra", "grid", "conflicted")
...
choice1_data <- data %>%
  filter(is.na(player.computer_choice1))

choice2_data <- data %>%
  filter(is.na(player.computer_choice1) & is.na(player.computer_choice2))

bet1_data <- data %>%
  filter(is.na(player.computer_choice1) & is.na(player.computer_bet_one))

bet2_data <- data %>%
  filter(is.na(player.computer_choice1) & 
         is.na(player.computer_bet_one) & 
         is.na(player.computer_choice2))
...
preprocess_switch_diff_data <- function(data, outcome_type, questionnaire_var, params) {
  # First apply the filtering as in original analysis
  filtered_data <- data %>%
    filter(
      player.computer_choice_one == 0,
      player.computer_choice_two == 0,
      ...
      player.player3_choice1_computer == 0,
      player.player4_choice1_computer == 0,
      player.player1_choice2_computer == 0,
      player.player2_choice2_computer == 0,
      player.player3_choice2_computer == 0,
      player.player4_choice2_computer == 0
    )

  # Calculate switches across trials, as in original analysis
  processed_data <- filtered_data %>%
    ...
    mutate(
      ...
      choice_switch_across_trials = case_when(
        is.na(lag(group.trial_number)) ~ NA_real_,
...
reversal_data <- data %>%
  select(
    ...
    player.player1_choice1_computer,
    player.player1_choice2_computer,
