// Simple Rescorla-Wagner RL model -- Zhang & Gläscher (2020) [10.1126/sciadv.abb4159]
data {
  ...
  int<lower=0,upper=1> chswitch[nSubjects,nTrials];     // choice switch, 0 or 1
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:Tsubj[s]) {
      ...
      chswitch[s,t] ~ bernoulli_logit(valfun2);

      if ( chswitch[s,t] == 0) {
        betfun2 = beta[6,s] + betfun1;
      } else if ( chswitch[s,t] == 1) {
  ...
  {// compute the log-likelihood
    for (s in 1:nSubjects) {
      ...
      for (t in 1:Tsubj[s]) {
        ...
        valfun2 = beta[2,s] + beta[3,s] * valdiff;
        log_likc2[s] = log_likc2[s] + bernoulli_logit_lpmf(chswitch[s,t] | valfun2);
