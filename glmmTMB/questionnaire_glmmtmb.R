...
run_model_pipeline <- function(data, params, output_path) {
  ...
  for(var in params$questionnaire_vars) {
    ...
    # Store median split p-values for main scale
    if(!is.null(analysis_results$main_results$median_split) && 
       !is.na(analysis_results$main_results$median_split$p_value)) {
      median_split_p_values[[paste0(var, "_main_median")]] <- 
        analysis_results$main_results$median_split$p_value
    }
    ...
    if(!is.null(analysis_results$subscale_results)) {
      for(subscale in names(analysis_results$subscale_results)) {
        # Store continuous model p-values
        interaction_p_values[[paste0(var, "_", subscale)]] <- 
          analysis_results$subscale_results[[subscale]]$raw_p
        
        # Store median split p-values for subscales
        if(!is.null(analysis_results$subscale_results[[subscale]]$median_split) &&
           !is.na(analysis_results$subscale_results[[subscale]]$median_split$p_value)) {
          median_split_p_values[[paste0(var, "_", subscale, "_median")]] <- 
            analysis_results$subscale_results[[subscale]]$median_split$p_value
        }
      }
    }
  ...
  # Perform FDR correction separately for continuous and median split p-values
  ...
  for(var in params$questionnaire_vars) {
    ...
    # Update median split adjusted p-value for main scale
    if(!is.null(all_results[[var]]$moderation_results$main_results$median_split) &&
       !is.null(all_median_adj_p_values)) {
      median_key <- paste0(var, "_main_median")
      if(median_key %in% names(all_median_adj_p_values)) {
        all_results[[var]]$moderation_results$main_results$median_split$adj_p <- 
          all_median_adj_p_values[median_key]
      }
    }
    ...
    if(!is.null(all_results[[var]]$moderation_results$subscale_results)) {
      for(subscale in names(all_results[[var]]$moderation_results$subscale_results)) {
        # Update continuous model p-value
        all_results[[var]]$moderation_results$subscale_results[[subscale]]$adj_p <- 
          all_adj_p_values[paste0(var, "_", subscale)]
        
        # Update median split p-value for subscale
        if(!is.null(all_results[[var]]$moderation_results$subscale_results[[subscale]]$median_split) &&
           !is.null(all_median_adj_p_values)) {
          median_sub_key <- paste0(var, "_", subscale, "_median")
          if(median_sub_key %in% names(all_median_adj_p_values)) {
            all_results[[var]]$moderation_results$subscale_results[[subscale]]$median_split$adj_p <- 
              all_median_adj_p_values[median_sub_key]
          }
        }
      }
    }
...
run_model_comparison <- function(model_formulas, data, params) {
  ...
  # Use AICtab to display model comparison
  aic_tab <- bbmle::AICtab(models, mnames = names(models), sort = FALSE)
...
run_individual_analysis <- function(processed_data, winning_model, var, params) {
  ...
  main_model <- tryCatch({
    model <- glmmTMB(formula(winning_model), data = processed_data,
                     REML = TRUE,
                     control = glmmTMBControl(optimizer = "optim"))
    model
  }, error = function(e) {
  ...
  # Get ANOVA results for main model - adjust for glmmTMB
  ...
  # Run median split analysis with updated model
    ...
    for(subscale in params$subscale_mapping[[var]]) {
      if(subscale %in% colnames(processed_data)) {
        ...
        subscale_model <- tryCatch({
          model <- glmmTMB(formula(winning_model), data = subscale_data,
                           REML = TRUE,
                           control = glmmTMBControl(optimizer = "optim"))
          model
        }, error = function(e) {
...
run_median_split_analysis <- function(data, winning_model, params) {
  ...
  # Fit the model with adapted formula using glmmTMB
  median_model <- tryCatch({
    glmmTMB(new_formula, data = prepped_data,
            REML = TRUE,
            control = glmmTMBControl(optimizer = "optim"))
  }, error = function(e) {
    ...
    tryCatch({
      simpler_formula <- as.formula(paste("outcome_value ~ consensus_level * direction * quest_group + (1|participant.id_in_session)"))
      message("Trying simpler model formula for median split...")
      glmmTMB(simpler_formula, data = prepped_data, 
              REML = TRUE,
              control = glmmTMBControl(optimizer = "optim"))
    }, error = function(e2) {
      message(paste("Error fitting simplified median split model:", e2$message))
      return(NULL)
    })
  })
...
check_model_diagnostics <- function(model, params) {
  # Check if we're dealing with a glmmTMB model
  if(inherits(model, "glmmTMB")) {
    # Get model frame more safely
    model_frame <- tryCatch({
      model$frame
    }, error = function(e) {
      warning("Could not access model frame directly, attempting to reconstruct")
      # Fallback to get the data used in the model
    ...
    residuals <- tryCatch({
      res <- residuals(model)
      if(length(res) == 0) {
        warning("Empty residuals, trying pearson residuals")
        residuals(model, type = "pearson")
      } else {
        res
      }
    }, error = function(e) {
      warning("Error extracting residuals: ", e$message)
      rep(NA, nrow(model_frame))
    })
    ...
    shapiro_test <- tryCatch({
      shapiro.test(shapiro_residuals[!is.na(shapiro_residuals)])
    }, error = function(e) {
      warning("Could not perform Shapiro-Wilk test: ", e$message)
      list(p.value = NA)
    })
    
    outliers <- tryCatch({
      boxplot.stats(residuals)$out
    }, error = function(e) {
      warning("Could not identify outliers: ", e$message)
      numeric(0)
    })
    ...
    fitted_vals <- tryCatch({
      ...
      warning("Could not extract fitted values: ", e$message)
      rep(NA, length(residuals))
    })
    
    # Calculate VIF for fixed effects more robustly
    vif_values <- tryCatch({
      # For glmmTMB models, carefully extract formula components
      if(is.null(model$formula)) {
        fixed_terms <- all.vars(formula(model)[[3]])
      } else {
        # Try to get terms from the fixed effects part of the formula
        fixed_formula <- model$modelInfo$allForm$conditional
        fixed_terms <- attr(terms(fixed_formula), "term.labels")
      }
      
      # Check if fixed_terms is NULL or empty
      if(is.null(fixed_terms) || length(fixed_terms) == 0) {
        warning("Could not extract fixed effect terms from model")
        return(NA)
      }
      
      # Reconstruct a formula for VIF calculation
      ...
      warning("Could not calculate VIF values: ", e$message)
      NA
    })
    
    # Calculate R² values - use performance package for glmmTMB
    r2_values <- tryCatch({
      r2 <- performance::r2_nakagawa(model)
      c(r2$R2_marginal, r2$R2_conditional)
    }, error = function(e) {
      # Fallback to MuMIn if performance package fails
      tryCatch(
        MuMIn::r.squaredGLMM(model),
        error = function(e2) {
          warning("Could not calculate R² values: ", e2$message)
          c(NA, NA)
        }
      )
    })
    
    # Check convergence - glmmTMB has different convergence checking
    convergence <- tryCatch({
      # Check optimizer convergence code
      if(is.null(model$sdr) || is.null(model$fit)) {
        warning("Incomplete model object, cannot check convergence")
        NA
      } else {
        conv_check <- model$sdr$pdHess && model$fit$convergence == 0
        !conv_check  # TRUE if there's a convergence issue
      }
    }, error = function(e) {
      warning("Could not check convergence status: ", e$message)
      NA
    })
    
    # Test homoscedasticity
    homo_test <- tryCatch({
      cor.test(abs(residuals), fitted_vals)
    }, error = function(e) {
      warning("Could not test homoscedasticity: ", e$message)
    ...
    ranef_summary <- tryCatch({
      ...
      warning("Could not summarize random effects: ", e$message)
      NULL
    })
    
    # Create DHARMa residuals for better diagnostics
    dharma_residuals <- tryCatch({
      DHARMa::simulateResiduals(model, n = 250)
    }, error = function(e) {
      warning("Could not create DHARMa residuals: ", e$message)
      NULL
    })
    
    # Analysis-specific checks
    switch_diff_diagnostics <- NULL
    within_trial_diagnostics <- NULL
    
    if(params$analysis_type == "switch_difference" && "switch_difference" %in% names(model_frame)) {
      # Additional checks specific to switch difference analysis
      switch_diff_diagnostics <- tryCatch({
        list(
          switch_diff_range = range(model_frame$switch_difference, na.rm = TRUE),
          switch_diff_distribution = shapiro.test(model_frame$switch_difference)
        )
      }, error = function(e) {
        warning("Error in switch difference diagnostics: ", e$message)
        NULL
      })
    } else if(params$analysis_type == "within_trial_switch" && "switch_vs_stay" %in% names(model_frame)) {
      ...
      within_trial_diagnostics <- tryCatch({
        ...
        warning("Error in within-trial switch diagnostics: ", e$message)
        NULL
      })
    }
    ...
    diagnostics <- list(
      residuals = list(
        ...
        n_residuals = sum(!is.na(residuals))
      ),
      model_fit = list(
      ...
      dharma = dharma_residuals
    )
    
    # Add analysis-specific diagnostics
    if(!is.null(switch_diff_diagnostics)) {
      diagnostics$switch_difference <- switch_diff_diagnostics
    } else if(!is.null(within_trial_diagnostics)) {
    ...
    # For lme4 models, use the original code with added error handling
    # Get model frame and terms
    model_frame <- tryCatch({
      model@frame
    }, error = function(e) {
      warning("Could not access model frame: ", e$message)
      data.frame()
    })
    ...
    model_terms <- tryCatch({
      ...
      warning("Could not extract model terms: ", e$message)
      NULL
    })
    ...
    residuals <- tryCatch({
      ...
      warning("Could not extract residuals: ", e$message)
      rep(NA, nrow(model_frame))
    })
    ...
    fixed_terms <- tryCatch({
      ...
      warning("Could not extract fixed term labels: ", e$message)
      NULL
    })
    ...
    r2_values <- tryCatch(
      MuMIn::r.squaredGLMM(model),
      error = function(e) {
        warning("Could not calculate R² values: ", e$message)
        c(NA, NA)
      }
    )
    ...
    convergence <- tryCatch({
      ...
      warning("Could not check convergence: ", e$message)
      NA
    })
    ...
    if(params$analysis_type == "switch_difference" && nrow(model_frame) > 0 && "switch_difference" %in% names(model_frame)) {
      # Additional checks specific to switch difference analysis
      switch_diff_diagnostics <- tryCatch({
        list(
          switch_diff_range = range(model_frame$switch_difference, na.rm = TRUE),
    ...
    } else if(params$analysis_type == "within_trial_switch" && nrow(model_frame) > 0 && "switch_vs_stay" %in% names(model_frame)) {
      # Additional checks specific to within-trial switch analysis
      within_trial_diagnostics <- tryCatch({
        list(
          switch_stay_counts = table(model_frame$switch_vs_stay),
...
format_results <- function(all_results, analysis_name, params) {
  ...
  # Create model descriptions outside the conditional
  model_explanations <- sprintf("m%d: %s", 
                                seq_along(first_result$model_results$descriptions),
                                first_result$model_results$descriptions)
  
  # Display AICtab results if available
  if(!is.null(first_result$model_results$aic_tab)) {
    output_text <- paste0(output_text,
                          "AIC Table:\n===========\n",
                          paste(capture.output(first_result$model_results$aic_tab), collapse = "\n"),
                          "\n\n")
    
    # Add model descriptions and winning model formula
    output_text <- paste0(output_text, 
                          "Model Descriptions:\n=================\n",
    ...
    # Create model comparison table (original code)
    ...
    # Add comparison section using model_comp_df
    output_text <- paste0(output_text, 
                          "MODEL COMPARISON:\n================\n",
  ...
  for(var in names(all_results)) {
    ...
    # NEW: Add section for median split results
...
format_subscale_results <- function(subscale_results, params) {
  ...
  for(subscale_name in names(subscale_results)) {
    ...
    # NEW: Add median split results
    if(!is.null(sub_result$median_split) && !is.na(sub_result$median_split$p_value)) {
      output <- paste0(output,
                       "\nMedian Split Interaction:\n",
                       "Raw p = ", format.pval(sub_result$median_split$p_value, digits = 3), "\n")
      
      if(!is.null(sub_result$median_split$adj_p)) {
        output <- paste0(output,
                         "FDR-adjusted p = ", format.pval(sub_result$median_split$adj_p, digits = 3), 
                         "\n")
      }
    }
